...
static bool ipcScanCmd(int argc, char *argv[], bool fRelay)
{
    ...
    {
        if (boost::algorithm::istarts_with(argv[i], "sfccoin:"))
        {
            ...
            try {
                ...
                {
                    printf("Sfccoin main() - boost interprocess exception #%d: %s\n", ex.get_error_code(), ex.what());
                    break;
                }
            }
        }
...
static void ipcThread(void* pArg)
{
    ...
    printf("Sfccoin ipcThread exited\n");
}

static void ipcThread2(void* pArg)
{
    printf("Sfccoin ipcThread started\n");
...
void ipcInit(int argc, char *argv[])
{
    ...
    try {
        ...
        printf("Sfccoin ipcInit() - boost interprocess exception #%d: %s\n", ex.get_error_code(), ex.what());
        return;
    }
